...
Counts the number of elements in an array where a specified field equals a given value. Supports nested field paths and performs case-sensitive comparison.

## Syntax

```jyro
CountIf(array, fieldName, value)
```
...
- **value** (any): The value to compare against using deep equality

## Returns

- **number**: The count of elements where the specified field equals the comparison value

## Description

The `CountIf` function searches through an array of objects and counts how many elements have a field that equals the specified value. The function supports nested field paths using dot notation, allowing you to compare deeply nested properties.

Comparison is performed using Jyro's deep equality semantics, which means complex objects are compared by their contents. String comparisons are case-sensitive.
